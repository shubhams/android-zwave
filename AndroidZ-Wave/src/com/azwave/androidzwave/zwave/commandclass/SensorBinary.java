...
public class SensorBinary extends CommandClass {

	public static final byte COMMAND_CLASS_ID = (byte) 0x30;
	public static final String COMMAND_CLASS_NAME = "COMMAND_CLASS_SENSOR_BINARY";

	public static final byte SENSOR_BINARY_CMD_GET = (byte) 0x02;
	public static final byte SENSOR_BINARY_CMD_REPORT = (byte) 0x03;
	...
	public boolean handleMsg(byte[] data, int length, byte instance) {
		if (SENSOR_BINARY_CMD_REPORT == data[0]) {
			ValueBool val = (ValueBool) getValue(instance, (byte) 0);
			if (val != null) {
				val.onValueRefreshed(data[1] != 0);
			}
			return true;
		}
		return false;
	...
	public void createVars(byte mInstance) {
		if (node != null) {
			node.getValueManager().createValueBool(ValueGenre.USER,
					getCommandClassId(), mInstance, (byte) 0, "Sensor", "",
